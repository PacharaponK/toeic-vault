...
### Step 5 — บันทึก Score ลง Daily Note และตรวจ Journal

```
อัปเดต Daily/[วันที่].md ดังนี้:

1. **Frontmatter scores:**
   part5_score: [x]/10
   part6_score: [x]/4
   part7_score: [x]/5
   topics_studied: [ชื่อหัวข้อที่เรียนวันนี้]

2. **ตาราง "สิ่งที่เรียนวันนี้":**
   เพิ่มแถวสำหรับทุก exercise ที่ทำวันนี้ พร้อม wikilink และคะแนน

3. **"จุดที่ยังไม่แม่น":**
   สรุปข้อที่ทำผิดในวันนี้ พร้อมกฎที่ต้องจำ

4. **"🔗 Notes ที่เกี่ยวข้องวันนี้":**
   เพิ่ม wikilink ไปยัง Rules note ทุกบทที่เรียนวันนี้

5. **ตรวจ Journal:**
   อ่าน Journal ที่ผู้ใช้เขียนในส่วน "✍️ Journal"
   - ถ้าพบข้อผิดพลาด (grammar / word choice / collocations):
     - แสดงประโยคเดิมด้วย ~~strikethrough~~ เก็บไว้ให้ครบ
     - แสดงประโยคที่แก้ถัดจากลูกศร →
     - ให้คำแนะนำสั้นๆ ภาษาไทย ว่าผิดตรงไหนและทำไม
     - คงสำนวนและ vocabulary ที่ผู้ใช้เลือกไว้ให้มากที่สุด ห้ามเขียนใหม่ทั้งประโยคถ้าไม่จำเป็น
   - ถ้าถูกต้อง: แจ้งว่าประโยคนั้น ✅ ไม่ต้องแก้ไข
```
